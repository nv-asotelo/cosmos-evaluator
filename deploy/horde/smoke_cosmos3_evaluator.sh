...
case "$NIM_MODEL_SIZE" in
  super) VLM_RUNTIME_ENDPOINT="${VLM_RUNTIME_ENDPOINT:-cosmos3-super-reasoner}" ;;
  nano) VLM_RUNTIME_ENDPOINT="${VLM_RUNTIME_ENDPOINT:-cosmos3-nano-reasoner}" ;;
...
curl -fsS -X POST "http://localhost:${CONTROL_PORT}/runtime/vlm/switch" \
  ...
  -d "{\"endpoint\":\"${VLM_RUNTIME_ENDPOINT}\"}"
